...
= Noise(噪声)
运算放大器电路表现出随机的内部噪声有两个原因:
- 由于半导体中电子空穴对的随机产生和重组
- 电阻器中电子的热搅动(thermal/Johnson noise)
= Signal-to-noise ratio(SNR, 信噪比)
$ "SNR"=20log_10 (("rms of signal" V_s)/("rms of noise" E_n)) $
rms:均方根
= 运放噪声模型
一般是用一个无噪声的运放输入端加上相等的噪声噪声源来建模,分为电压噪声$e_n$和
$i_n$.
- 电压噪声在(+/非反相输入端)端加一个, 一个具有频谱密度的量(单位:$V\/sqrt("HZ")$)
- 电流噪声作用在两个输入端(分别流入),单位是$A\/sqrt("HZ")$
== 电阻的热噪声
$ E_r=sqrt(4 R k T Delta f) $
- k:玻尔兹曼常数
- T:热力学温度
- $Delta f$:噪声的带宽(决定了观测噪声的频率范围,带宽越宽,观测到的总噪声越大.)
- R:电阻值
=== 关于带宽
- 白噪声:在频谱上对于所有频率具有相同的幅度
#figure(caption: [白噪声频谱图])[#image("2024-12-23-18-33-11.png")]
#figure(caption: [白噪声通过低通滤波器的结果, 以及NEB(Noise Equivalent Bandwidth)的定义])[#image("2024-12-23-18-37-36.png")]
运放的NEB:\
#figure(caption: [闭环频率响应])[#image("2024-12-23-19-05-00.png")]
下降了3dB就是cut-off frequency.
#cmarker.render("这段内容描述了运算放大器（Op-Amp）的**闭环频率响应**以及增益带宽积（Gain-Bandwidth Product, GBW）和噪声等效带宽（Noise Equivalent Bandwidth, NEB）的相关概念。以下是详细解释：
...
$n$由于不同的运放级别有不同的值
#table(columns: 2, align: center)[Order][n][1][1.57][2][1.22][3][1.16][4][1.13][5][1.12]
= Total RMS input noise
